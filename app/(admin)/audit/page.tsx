...
import {
  ...
  onSnapshot, where, doc, getDoc, getDocs,
...
import {
  History, Loader2, Download, Search,
  CheckCircle2, XCircle, AlertTriangle,
  UserPlus, RefreshCw, Edit3, Trash2,
...
type ActionType =
  ...
  | "REGISTER"   | "RENEW"       | "APPROVE"
  | "EDIT"       | "DELETE"      | "STATUS_CHANGE"
  | "EXPORT"     | "IMPORT";
...
const ACTION_META: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  REGISTER:      { label: "Registered",     color: "bg-green-100 text-green-700 border-green-200",       icon: <UserPlus      className="h-3 w-3" /> },
  STATUS_CHANGE: { label: "Status Changed", color: "bg-purple-100 text-purple-700 border-purple-200",    icon: <RefreshCw     className="h-3 w-3" /> },
  RENEW:         { label: "Renewed",        color: "bg-emerald-100 text-emerald-700 border-emerald-200", icon: <RefreshCw     className="h-3 w-3" /> },
  APPROVE:       { label: "Approved",       color: "bg-teal-100 text-teal-700 border-teal-200",          icon: <CheckCircle2  className="h-3 w-3" /> },
  EDIT:          { label: "Edited",         color: "bg-orange-100 text-orange-700 border-orange-200",    icon: <Edit3         className="h-3 w-3" /> },
  DELETE:        { label: "Deleted",        color: "bg-red-100 text-red-700 border-red-200",             icon: <Trash2        className="h-3 w-3" /> },
  EXPORT:        { label: "Exported",       color: "bg-indigo-100 text-indigo-700 border-indigo-200",    icon: <Download      className="h-3 w-3" /> },
  IMPORT:        { label: "Imported",       color: "bg-cyan-100 text-cyan-700 border-cyan-200",          icon: <FileText      className="h-3 w-3" /> },
  AUTH_LOGIN:    { label: "Logged In",      color: "bg-blue-100 text-blue-700 border-blue-200",          icon: <LogIn         className="h-3 w-3" /> },
  AUTH_LOGOUT:   { label: "Logged Out",     color: "bg-slate-100 text-slate-600 border-slate-200",       icon: <LogOut        className="h-3 w-3" /> },
  AUTH_FAILED:   { label: "Auth Failed",    color: "bg-red-100 text-red-700 border-red-200",             icon: <AlertTriangle className="h-3 w-3" /> },
};
...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(async (user) => {
      ...
      if (snap.exists()) {
        ...
        setUserProfile({ uid: user.uid, role: d.role, entity: d.entity, name: d.name });
      }
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => { console.error("Audit log error:", err); setLoading(false); },
    );
    return () => unsub();
  }, [userProfile]);
  ...
  const stats = useMemo(() => ({
    total:         logs.length,
    today:         logs.filter((l) => toDate(l.timestamp).toDateString() === new Date().toDateString()).length,
  ...
  const filtered = useMemo(() => logs.filter((l) => {
    ...
    const matchSearch = !search || [l.target, l.RIN ?? "", adminName, l.action, l.district ?? ""]
  ...
  // ── Export CSV ────────────────────────────────────────────────────────────
  const handleExport = () => {
    const headers = ["Timestamp", "Registrar", "Role", "Action", "Rider / Applicant", "RIN", "District", "Status"];
    ...
    const a    = Object.assign(document.createElement("a"), {
      href: url, download: `audit-${format(new Date(), "yyyy-MM-dd")}.csv`,
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          { label: "Total Entries",  value: stats.total,         color: "text-slate-900"  },
          { label: "Today",          value: stats.today,         color: "text-blue-600"   },
          { label: "Registrations",  value: stats.registrations, color: "text-green-700"  },
          { label: "Renewals",       value: stats.renewals,      color: "text-emerald-700"},
          { label: "Failed Actions", value: stats.failed,        color: "text-red-600"    },
        ].map((s) => (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50 border-b">
                <TableHead className="font-bold text-slate-600 text-xs w-36">Timestamp</TableHead>
                <TableHead className="font-bold text-slate-600 text-xs">Registrar</TableHead>
                <TableHead className="font-bold text-slate-600 text-xs">Action</TableHead>
                <TableHead className="font-bold text-slate-600 text-xs">Rider / Applicant</TableHead>
            ...
            <TableBody>
              {filtered.length > 0 ? filtered.map((log) => {
                ...
                const adminName = adminMap[log.adminUid] ?? "Unknown";
                const adminRole = roleMap[log.adminUid]  ?? log.adminRole ?? "";
                ...
                return (
                  <TableRow
                    ...
                    className={`border-b transition-colors hover:bg-slate-50 ${
                      ...
                      log.status === "warning" ? "bg-yellow-50/40" : ""
                    }`}
                  >
                    {/* Timestamp */}
                    ...
                    {/* Action */}
                    <TableCell className="py-3">
                      <Badge variant="outline" className={`text-[10px] font-semibold gap-1 border ${meta.color}`}>
                        {meta.icon}
                        {meta.label}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="py-3">
                      {isAuth ? (
                        ...
                        <>
                          ...
                          {log.RIN && (
                            <p className="text-[10px] font-mono text-green-700 mt-1">{log.RIN}</p>
                          )}
                          {log.district && (
                            <p className="text-[10px] text-slate-400 mt-0.5">{log.district}</p>
                          )}
                        </>
                      )}
                    </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={5} className="py-16 text-center">
                    ...
                    <p className="text-slate-400 font-semibold text-sm">
                      {search || typeFilter !== "ALL" ? "No entries match your filter" : "No audit logs yet"}
                    </p>
