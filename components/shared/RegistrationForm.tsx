      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
          ...
          <div>
            ...
            <p className="text-sm text-green-700">RIN generated. Print or save this certificate for the rider's records.</p>
          </div>
        </div>
        ...
        <div className="bg-white border-2 border-slate-200 rounded-2xl overflow-hidden shadow-xl">
          ...
          <div className="bg-green-700 px-7 py-6 text-white flex items-start justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-green-200 mb-1">Republic of Ghana · CTS Africa</p>
              <h1 className="text-xl font-bold leading-tight">Rider Identification Number</h1>
              <p className="text-sm text-green-200 mt-1">This certifies successful registration of a commercial rider</p>
            </div>
          ...
          <div className="bg-slate-50 border-b border-slate-200 px-7 py-5 grid grid-cols-[1fr_auto_auto] gap-4 items-center">
            <div className="bg-green-50 border-2 border-green-600 rounded-xl px-6 py-5">
              <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-green-700 mb-2">Rider Identification Number</p>
              <p className="text-3xl font-black font-mono tracking-widest text-green-900 select-all">{generatedRIN}</p>
              <p className="text-xs text-green-600 mt-2 italic">Issued {issuedDate} at {issuedTime}</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              {photoPreview
                ? <img src={photoPreview} alt="Passport" className="w-20 h-24 object-cover rounded-lg border-2 border-slate-200 shadow" />
                : <div className="w-20 h-24 bg-slate-200 rounded-lg flex items-center justify-center text-slate-400 text-xs font-semibold">No Photo</div>}
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Photo</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              {qrCodeUrl
                ? <img src={qrCodeUrl} alt="QR" className="w-24 h-24 object-contain border-2 border-green-300 rounded-lg p-1 bg-white" />
                : <div className="w-24 h-24 flex flex-col items-center justify-center gap-1 border-2 border-dashed border-slate-300 rounded-lg">
                    ...
                    <p className="text-[9px] text-slate-400 text-center">Generating...</p>
                  </div>}
              <p className="text-[9px] font-bold uppercase tracking-wider text-green-700">Scan to Verify</p>
            </div>
          </div>
          ...
          <div className="px-7 py-6 space-y-6">
            ...
            <CertSection title="Vehicle Details">
              <div className="grid grid-cols-3 gap-4">
                ...
                <CertField label="Driver's License" value={data.driversLicenseNumber} />
                <CertField label="License Expiry"   value={data.licenseExpiryDate} />
              </div>
            </CertSection>
            ...
            <p className="text-[10px] text-slate-400 text-center pt-2 border-t border-slate-100">
              Computer-generated document — valid without a handwritten signature. Scan QR code to verify authenticity online.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Button
            size="lg"
            onClick={() => printCertificate(data, generatedRIN, qrCodeUrl, photoPreview)}
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-xs font-black border-2 transition-all ${
                      isDone    ? "bg-green-700 border-green-700 text-white"      :
                      isCurrent ? "bg-white border-green-600 text-green-700"     :
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-bold leading-none ${isCurrent ? "text-green-800" : "text-slate-600"}`}>
                        {step.title}
                      </p>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap shrink-0 border transition-all ${
                      isCurrent ? "bg-green-700 text-white border-green-700" :
            ...
            >
              ...
              <div className="flex justify-between items-center pt-8 border-t border-slate-200">
                <Button
                  ...
                  disabled={currentStep === 1 || isSubmitting} className="gap-2"
                >
...
function CertSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[10px] font-bold uppercase tracking-widest text-green-700 mb-3 pb-2 border-b border-green-100">{title}</p>
      {children}
    </div>
  );
}
