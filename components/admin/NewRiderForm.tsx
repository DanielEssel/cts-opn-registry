...
export default function NewRiderForm({ onSuccess }: NewRiderFormProps) {
  ...
  async function onSubmit(values: RiderFormValues) {
    ...
    try {
      const PCRAA = await createRider(values);
      toast.success(`Registration Successful! PCRAA: ${PCRAA}`);
      form.reset();
      setStep(1);
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="flex gap-4">
                ...
                <Button type="submit" disabled={isSubmitting} className="w-full bg-green-600 hover:bg-green-700">
                  {isSubmitting ? <Loader2 className="animate-spin" /> : "Complete Registration & Issue PCRAA"}
                </Button>
              </div>
            </div>
          )}
