...
import {
  ...
  limit, doc, getDoc, serverTimestamp, updateDoc,
...
async function uploadPassportPhoto(file: File, idNumber: string): Promise<string | null> {
  try {
    ...
    console.log(`📷 Uploading photo: ${fileName} (${Math.round(file.size / 1024)} KB)`);
...
async function generateAndUploadQRCode(RIN: string, riderId: string): Promise<string | null> {
  try {
    const baseUrl   = typeof window !== "undefined" ? window.location.origin : "";
    const verifyUrl = `${baseUrl}/verify/${RIN}`;
    ...
    const snapshot   = await uploadBytes(storageRef, qrFile, { contentType: "image/png" });
    const url        = await getDownloadURL(snapshot.ref);
...
export async function saveRiderRegistration(
  ...
  try {
    ...
    const message =
      err?.details?.message ?? err?.message ?? "Registration failed. Please try again.";
    return { success: false, RIN: "", riderId: "", qrCodeUrl: "", error: message };
  }
}
...
export async function updateRider(
  ...
  try {
    await updateDoc(doc(db, "riders", riderId), { ...updates, updatedAt: serverTimestamp() });
    return true;
  } catch (err) {
...
export async function lookupByRIN(RIN: string): Promise<RiderLookupResult> {
  if (!isValidRIN(RIN)) {
    return { found: false, error: "Invalid RIN format. Expected: GAP-0001-KR0326" };
  }
...
export async function getAllRiders(): Promise<(RiderRecord & { id: string })[]> {
  const snap = await getDocs(query(collection(db, "riders"), orderBy("createdAt", "desc")));
  return snap.docs.map((d) => ({ id: d.id, ...(d.data() as RiderRecord) }));
}

export async function getRidersByOperator(uid: string): Promise<(RiderRecord & { id: string })[]> {
  const snap = await getDocs(
    query(collection(db, "riders"), where("createdBy", "==", uid), orderBy("createdAt", "desc"))
  );
  return snap.docs.map((d) => ({ id: d.id, ...(d.data() as RiderRecord) }));
}

export async function getRidersByDistrict(district: string): Promise<(RiderRecord & { id: string })[]> {
  const snap = await getDocs(
    query(collection(db, "riders"), where("districtMunicipality", "==", district), orderBy("createdAt", "desc"))
  );
  return snap.docs.map((d) => ({ id: d.id, ...(d.data() as RiderRecord) }));
}
...
export async function getRidersByStatus(
  ...
  const snap = await getDocs(
    query(collection(db, "riders"), where("status", "==", status), orderBy("createdAt", "desc"))
  );
  return snap.docs.map((d) => ({ id: d.id, ...(d.data() as RiderRecord) }));
}
...
export const daysUntilExpiry        = (d: string) => Math.ceil((new Date(d).getTime() - Date.now()) / 86_400_000);
export const isLicenseExpired       = (d: string) => new Date(d) < new Date();
export const daysUntilLicenseExpiry = (d: string) => Math.ceil((new Date(d).getTime() - Date.now()) / 86_400_000);
